...
- (int)startRecord
{
    ...
    NSString *filename = [NSString stringWithFormat:@"%@%@", date, @".avi"];
    
    //Document文件夹：用来保存应由程序运行时生成的需要持久化的数据， iTunes会自动备份该目录（苹果公司建议将程序中创建的和浏览过的程序存放在这里，iTunes在备份和回复时会自动包含此目录）。
    ...
    _recordFilePath = [documentsPath stringByAppendingPathComponent:filename]; //Add the file name
    const char * file_Name = [_recordFilePath UTF8String];
    int32_t ret = ijkmp_startRecord(_mediaPlayer, file_Name);
...
#pragma mark - CONVERT_AVI_TO_MP4
//https://www.jianshu.com/p/0258ed2133ff  ARC 的__autoreleasing相关知识点
- (void) convertAVItoMP4WithInputPath:(NSString *)inputFilePath
                    outputUrl:(NSURL __strong** ) outputvideoUrl
{
    // Save to the album
    //NSString *inputFilePath = _recordFilePath;
    //output file name
    NSDate *today = [NSDate date];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd_HH-mm-ss"];
    NSString *date = [dateFormatter stringFromDate:today];
    NSString *outputFilename = [NSString stringWithFormat:@"%@%@", date, @".MP4"];
    //output file path = path + name
    //tmp：保存应用程序的临时文件夹，使用完毕后，将相应的文件从这个目录中删除，如果空间不够，系统也可能会删除这个目录下的文件，iTunes不会同步这个文件夹，在iPhone重启的时候，该目录下的文件会被删除。
    NSString *documentsPath = NSTemporaryDirectory();
    NSString *outputFilePath = [documentsPath stringByAppendingPathComponent:outputFilename]; //Add the output file name
    //NSURL *outputvideoUrl = [NSURL fileURLWithPath:outputFilePath];
    *outputvideoUrl = [NSURL fileURLWithPath:outputFilePath];

    //ffmpeg -i input.avi -pix_fmt yuv420p -qscale:v 1 output.mp4
    NSString *command_str = [NSString stringWithFormat:@"ffmpeg -i %@ -pix_fmt yuv420p -qscale:v 1 %@", inputFilePath, outputFilePath];
    // 分割字符串
...
- (int)stopRecord
{
    ...
    int32_t ret = ijkmp_stopRecord(_mediaPlayer);
    if(ret>=0) {
        // Save to the album
        NSString *inputFilePath = _recordFilePath;
...
*/
        ...
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            ...
            //iOS9之后加入的，它是在原来iOS8的基础上给PHAssetChangeRequest添加了一个存取图片or视频到相册的子类PHAssetCreationRequest。重点是经过测试前面三种保存图片到相册的方法都会导致图片被压缩，只有第四种不会。第四种可以保存原图，前面三种保存图片的方式默认会被压缩。
            //add by Eason - Avi To MP4 - 20181212
            NSLog(@"[stopRecord]\n");
            NSLog(@"[stopRecord] File path %@", inputFilePath);
/*
            //
            ...
            //"2018-12-19_10-33-02.avi"
            //NSString *inputfilepath= [documentsPath stringByAppendingPathComponent:@"2018-12-19_10-33-02.avi"];
            NSURL *outputvideoUrl = [NSURL fileURLWithPath:outputFilePath];

            if ([[NSFileManager defaultManager] fileExistsAtPath:outputFilePath]){
                //這裡放置如果檔案存在時的程式
                NSLog(@"stopRecord outputFilePath = %@\n檔案存在！", outputFilePath);
            }else{
                NSLog(@"stopRecord outputFilePath = %@\n檔案不存在！", outputFilePath);
            }
            NSLog(@" Saved with iOS9 Photos PHAssetResourceCreationOptions\n");

*/

            [self convertAVItoMP4WithInputPath:inputFilePath outputUrl:&outputvideoUrl];

            NSLog(@"stopRecord outputvideoUrl = %@\n", outputvideoUrl);
            NSLog(@" Saved with iOS9 Photos PHAssetResourceCreationOptions\n");
            ...
            if ( ! success ) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath: outputFilePath]){
                    //這裡放置如果檔案存在時的程式
                    NSLog(@"stopRecord success 檔案存在！");

                }else{
                    NSLog(@"stopRecord success 檔案不存在！");

                }
                NSLog(@" Saved Success %@", outputFilePath);*/
                NSLog(@" Saved Success %@", outputvideoUrl);
                //[self clearTmpDirectory];
            }
        }];
    }
...
- (int)isRecording
{
    ...
    int32_t ret = ijkmp_isRecording(_mediaPlayer);
