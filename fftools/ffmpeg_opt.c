...
static void add_input_streams(OptionsContext *o, AVFormatContext *ic)
{
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        ist->decoder_opts = filter_codec_opts_e(o->g->codec_opts, ist->st->codecpar->codec_id, ic, st, ist->dec);
...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    if (!ic) {
        print_error_e(filename, AVERROR(ENOMEM));
        exit_program(1);
    }
    ...
    if (find_stream_info) {
        AVDictionary **opts = setup_find_stream_info_opts_e(ic, o->g->codec_opts);
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    if (ost->enc) {
        ...
        ost->encoder_opts  = filter_codec_opts_e(o->g->codec_opts, ost->enc->id, oc, st, ost->enc);
        ...
        ost->encoder_opts = filter_codec_opts_e(o->g->codec_opts, AV_CODEC_ID_NONE, oc, st, NULL);
    }
...
static int opt_streamid(void *optctx, const char *opt, const char *arg)
{
    ...
    *p++ = '\0';
    ...
    o->streamid_map = grow_array_e(o->streamid_map, sizeof(*o->streamid_map), &o->nb_streamid_map, idx+1);
...
loop_end:
    ...
    if (oc->oformat->flags & AVFMT_NEEDNUMBER) {
        if (!av_filename_number_test(oc->filename)) {
            print_error_e(oc->filename, AVERROR(EINVAL));
            exit_program(1);
        }
    }
